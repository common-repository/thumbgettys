<?php
/**
 * ThumbGettyss PHP class for {@link http://ThumbGettys}
 *
 * Author: Mr. Thumb - {@link http://www.thumbgettys.com/}
 * License: GPL version 3 or any later version
 * License info: {@link http://www.gnu.org/licenses/gpl.txt}
 *
 * API specification and examples: {@link http://ThumbGettys/api}
 *
 * Version: 1.7.7
 * Date: 24-Feb-2013
 *
 */
if( !defined('ThumbGettys_INIT') ) die( 'Please, do not access this page directly.' );
...
class ThumbGettys
{
	...
	// Personal access key
	// Register on http://my.ThumbGettys to get your own key
	var $access_key = '';

	// ThumbGettys url address
	var $url;
	...
	// Link ThumbGettyss to an exit "goodbye" page
	...
	var $image_class = 'ThumbGettyss_plugin';

	// Associative array of custom service images
	// key - (string) error code. For complete list of error codes see http://www.thumbgettys.com/error-codes
	...
	var $service_images = array(
				...
				// '0x0'	=> 'http://domain.tld/image-queued.jpg',	// ThumbGettys queued
				// '0x12'	=> 'http://domain.tld/image-bad-host.jpg',	// Invalid remote host
			);

	// Add custom params to ThumbGettys request, they will be added to request URL
	// http://www.thumbgettys.com/api
	var $args = array( 'type' => 'json' );

	var $dispatcher = 'http://get.ThumbGettys/?';


	// Internal
	protected $_name = 'ThumbGettyss PHP';
	...
	{
		$this->debug_disp('&lt;&lt;== Getting the ThumbGettys ==&gt;&gt;');
...
LvX4yglOQdGPf3juCHnJhcUJKmiOltGD2CyAAAMA9pVY15kNU24AAAAASUVORK5CYII=" alt="" title="Reload this ThumbGettys" /></a>';

					$output = $reload_link.$output;
				}
				return '<span class="ThumbGettyss_plugin">'.$output.'</span>';
			}
			else
			{
	...
	{
		// Set user agent
		@ini_set( 'user_agent', $this->_name.' v'.$this->_version.' (+http://www.ThumbGettys)' );
	...
	function check_dir()
	{
		...
		{
			$this->debug_disp('ThumbGettyss directory does not exist or is not writable.');
			return false;
		}
